...
*2019 data
local using= "${gsdData}/NGA-GHS2019-HHData.dta"
capture confirm file "`using'"
if _rc != 0 {
	*data directory
...
if (1==2) {
	*check consistency
	use "${gsdData}/NGA-GHS2016-HHData.dta", clear
	egen x = rowtotal(xfood* xnonfood* xdurables)
	keep hhid strata urban cluster weight hhsize x
	gen y = 2016
	save "${gsdTemp}/NGA-2016.dta", replace

	use "${gsdData}/NGA-GHS2019-HHData.dta", clear
	egen x = rowtotal(xfood* xnonfood* xdurables)
	keep hhid strata urban cluster weight hhsize x
	gen y = 2019
	append using "${gsdTemp}/NGA-2016.dta"
}