...
run "${gsdDo}/fRCS.do"
*Calculating hhsize:
use "${gsdDataBox}/sect1_harvestw3.dta", clear
...
use "${gsdDataBox}/sect10a_harvestw3.dta", clear 
gen item_cd_oh = item_cd
labmask item_cd_oh, val(item_cd) decode
drop item_cd
label drop item_cd
append using "${gsdDataBox}/sect10b_harvestw3.dta"
...
drop item_cd_oh
merge m:1 hhid using "${gsdDataBox}/HHTrack.dta", keep(match) keepusing(wt_wave3) nogen
