...
*		local M=4
		...
		*get total food consumption
		bysort hhid: egen  xfood_t = sum(xfood)
		*get reduced food consumption
		gen  xfred = xfood if itemred
		quiet: bysort hhid: egen  xfood_r = sum(xfred)
		drop xfred
		quiet: clonevar oxfood=xfood // Keep the original value
		*randomly assign module to households
		quiet: replace xfood = . if (itemmod>0) & (itemmod!=hhmod)
		quiet: bysort hhid: egen  xfood_t2 = sum(xfood) // The total of the observed
		save "`lc_sdTemp'/food-consumption.dta", replace // ST added
		*aggregate by modules; use itemmod to consider core module
		collapse (sum) xfood oxfood (mean) xfood_t xfood_t2 xfood_r (firstnm) weight, by(hhid hhmod itemmod)
		keep hhid hhmod xfood xfood_t xfood_r itemmod weight oxfood xfood_t2 
		ren xfood* xfcons*
		ren oxfood* oxfcons*
		quiet: reshape wide xfcons oxfcons, i(hhid hhmod weight xfcons_t xfcons_t2 xfcons_r) j(itemmod)
*		quiet: reshape wide xfcons oxfcons, i(hhid) j(itemmod)
		...
		*get reduced food consumption
		...
		drop xnonfred
		quiet: clonevar oxnonfood=xnonfood // Keep the original value
		*randomly assign module to households
		...
		*aggregate by modules; use itemmod to consider core module
		collapse (sum) xnonfood oxnonfood (mean) xnonfood_t xnonfood_t2 xnonfood_r, by(hhid hhmod itemmod)
		keep hhid xnonfood xnonfood_t xnonfood_r hhmod itemmod xnonfood oxnonfood xnonfood_t2
		ren xnonfood* xnfcons*
		ren oxnonfood* oxnfcons*
		quiet: reshape wide xnfcons oxnfcons, i(hhid hhmod xnfcons_t xnfcons_t2 xnfcons_r) j(itemmod)
		*ensure zero entries are not missing
		forvalues jmod = 1/`M' {
		...
		*get per capita variables
		ren xfcons_t2 ofcons
		ren xnfcons_t2 onfcons
		ren (x*cons_t x*cons_r) (c*cons r*cons)
		foreach v of varlist xfcons* xnfcons* oxfcons* oxnfcons* cfcons cnfcons rfcons rnfcons ofcons onfcons  {
		...
		*prepare check variables
		...
		quiet: gen ocons_pc = ofcons_pc + onfcons_pc + xdurables_pc
		rename o* f*
		...
		*Prepare item level database to get item shares
		...
		rename oxfood fcons_value
		label var fcons_value "Full Expenditure (1,000 shillings/household/month)"
		rename hhmod mod_hh 
...
program define RCS_simulate
	...
	*				tobit xfcons`imod'_pc xfcons0_pc xnfcons0_pc xdurables_pc `model' i.cluster [aweight=weight], ll(0) log // With cluster-level dummies
					tobit xfcons`imod'_pc xfcons0_pc xnfcons0_pc xdurables_pc `model1'           [aweight=weight], ll(0) log // No cluster-level dummies
    ...
    *				tobit xnfcons`imod'_pc xfcons0_pc xnfcons0_pc xdurables_pc `model' i.cluster [aweight=weight], ll(0) log // With cluster-level dummies 
					tobit xnfcons`imod'_pc xfcons0_pc xnfcons0_pc xdurables_pc `model1'          [aweight=weight], ll(0) log // No cluster-level dummies
					mata: cond(st_matrix("e(V)"))
    ...
    *				reg xfcons`imod'_pc i.pxfcons0_pc i.pxnfcons0_pc i.pxdurables_pc `model' i.cluster [aweight=weight] // With cluster-level dummies
					reg xfcons`imod'_pc i.pxfcons0_pc i.pxnfcons0_pc i.pxdurables_pc `model1'           [aweight=weight] // No cluster-level dummies
					predict y`imod'_pc if xfcons`imod'_pc>=.
	*				replace xfcons`imod'_pc = exp(y`imod'_pc) if xfcons`imod'_pc>=.
					replace xfcons`imod'_pc = max(y`imod'_pc,0) if xfcons`imod'_pc>=.
					drop y`imod'_pc
	...
	*				reg xnfcons`imod'_pc i.pxfcons0_pc i.pxnfcons0_pc i.pxdurables_pc `model' i.cluster [aweight=weight] // With cluster-level dummies
					reg xnfcons`imod'_pc i.pxfcons0_pc i.pxnfcons0_pc i.pxdurables_pc `model1'           [aweight=weight] // No cluster-level dummies
					predict y`imod'_pc if xnfcons`imod'_pc>=.
	*				replace xnfcons`imod'_pc = exp(y`imod'_pc) if xnfcons`imod'_pc>=.
					replace xnfcons`imod'_pc = max(y`imod'_pc,0) if xnfcons`imod'_pc>=.
					drop y`imod'_pc
	...
	*				replace xnfcons`imod'_pc = exp(y`imod'_pc) if xnfcons`imod'_pc>=.
				...
				forvalues imod = 1/`M' {
					*food
					...
					probit  dxfcons`imod'_pc i.pxfcons0_pc i.pxnfcons0_pc i.pxdurables_pc `model1'           [pweight=weight] // part I specification
					predict p`imod'_pc if xfcons`imod'_pc==.
					regress xfcons`imod'_pc i.pxfcons0_pc i.pxnfcons0_pc i.pxdurables_pc `model1' [pweight=weight] if dxfcons`imod'_pc==1 // part II specification
					predict y`imod'_pc if xfcons`imod'_pc==.
					...
					*non-food
					...
					probit  dxnfcons`imod'_pc i.pxfcons0_pc i.pxnfcons0_pc i.pxdurables_pc `model1'           [pweight=weight] // part I specification
					predict p`imod'_pc if xnfcons`imod'_pc==.
					regress xnfcons`imod'_pc i.pxfcons0_pc i.pxnfcons0_pc i.pxdurables_pc `model1' [pweight=weight] if dxnfcons`imod'_pc==1 // part II specification
					predict y`imod'_pc if xnfcons`imod'_pc==.
				...
				forvalues imod = 1/`M' {
					...
					*This will detect and record when the predicted mean of each case above is close (or the smallest) to the sample mean.  
					...
					replace xfcons`imod'_pc = (y`imod'_pc) if xfcons`imod'_pc==. 				// Use (unconditional) expection of Y  when Y!=0.
					replace xfcons`imod'_pc =  max(xfcons`imod'_pc, 0) if oxfcons`imod'_pc==. // Replace (small number of) negatives with zero
					drop y`imod'_pc p`imod'_pc order p _*
					...
					*This will produce all cases when the number of households with zero will increase one by one.
					...
					replace xnfcons`imod'_pc = (y`imod'_pc) if xnfcons`imod'_pc==.               // Use (unconditional) expection of Y  when Y!=0.
					replace xnfcons`imod'_pc =  max(xnfcons`imod'_pc, 0) if oxnfcons`imod'_pc==. // Replace (small number of) negatives with zero
					drop y`imod'_pc p`imod'_pc order p _*
...
/*				
				**Naive method to remove negative**	
				if ("`smethod'"=="MImvn") {
					*Multi-variate normal imputation using MCMC
					mi impute mvn xfcons1_pc xfcons2_pc xfcons3_pc xfcons4_pc xnfcons1_pc xnfcons2_pc xnfcons3_pc xnfcons4_pc = i.pxfcons0_pc i.pxnfcons0_pc i.pxdurables_pc `model', add(`nI') burnin(1000)
					...
					foreach cat in f nf {
						forvalues i=1/4 {
							*need to take floor to make sure rounding does not result in negative averages
							forvalues j=1/`nI' {
							*Simply replace negative imputation with zero
							replace _`j'_x`cat'cons`i'_pc=0 if _`j'_x`cat'cons`i'_pc<0
												}
										}
										}	
											}				
	...
	foreach smethod of local lmethod {
		...
		*extract consumption
		forvalues isim = 1/`N' {
			...
			*estimations
			if inlist("`smethod'","tobit","tobit2","tobit3","twopart","twopart2","twopart3") {
				file write fh "`isim'" _tab "1"
				foreach id of local xhh {
					quiet: summ xcons_pc if hhid==`id'
					file write fh _tab (r(mean))
			...
			*estimations
			if inlist("`smethod'","twopartII1","twopartII2","twopartII3") {
				file write fh "`isim'" _tab "1"
				foreach id of local xhh {
					quiet: summ xcons_pc if hhid==`id'
					file write fh _tab (r(mean))
