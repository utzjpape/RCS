...
rename rururb urban 
collapse urban, by(hhid)
label value urban rururb
save "`lc_sdTemp'/consumption_aggregated_data_urban.dta",replace
...
foreach cat of local lis {
	...
	*egen gr=group(strata ea block hh)
	...
	merge m:1 hhid using "`lc_sdTemp'/consumption_aggregated_data_urban.dta"
	keep if _m==3
	drop _m
	gen uprice=.
