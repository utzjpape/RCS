...
program define RCS_assign
	...
	*	recode module_nf (3=4) (4=3) (1=2) (2=1) if cluster > 50
		...
		*ensure zero entries are not missing
		save "`lc_sdTemp'/hh-food-consumption_pre.dta", replace
		forvalues jmod = 1/`M' {
		...
		*ensure zero entries are not missing
		save "`lc_sdTemp'/hh-nonfood-consumption_pre.dta", replace
		forvalues jmod = 1/`M' {
		...
		*create percentiles
		foreach v of varlist xfcons0_pc xnfcons0_pc  {
			xtile p`v' = `v' [pweight=weight], nquantiles(4)
		}

		*prepare check variables
		quiet: gen ccons_pc = cfcons_pc + cnfcons_pc 
		quiet: gen rcons_pc = rfcons_pc + rnfcons_pc 
...
program define RCS_simulate
	...
	*				tobit xfcons`imod'_pc xfcons0_pc xnfcons0_pc xdurables_pc `model' i.cluster [aweight=weight], ll(0) log // With cluster-level dummies
					tobit xfcons`imod'_pc xfcons0_pc xnfcons0_pc `model'           [aweight=weight], ll(0) log // No cluster-level dummies
    ...
    *				tobit xnfcons`imod'_pc xfcons0_pc xnfcons0_pc xdurables_pc `model' i.cluster [aweight=weight], ll(0) log // With cluster-level dummies 
					tobit xnfcons`imod'_pc xfcons0_pc xnfcons0_pc  `model'          [aweight=weight], ll(0) log // No cluster-level dummies
					mata: cond(st_matrix("e(V)"))
	...
	*				replace xnfcons`imod'_pc = exp(y`imod'_pc) if xnfcons`imod'_pc>=.
				...
				forvalues imod = 1/`M' {
					*food
	*				tobit xfcons`imod'_pc xfcons0_pc xnfcons0_pc xdurables_pc `model' i.cluster [aweight=weight], ll(0) log // With cluster-level dummies
					tobit xfcons`imod'_pc xfcons0_pc xnfcons0_pc  `model2'           [aweight=weight], ll(0) log // No cluster-level dummies
					mata: cond(st_matrix("e(V)"))
					predict y`imod'_pc if xfcons`imod'_pc>=.
	*				replace xfcons`imod'_pc = exp(y`imod'_pc) if xfcons`imod'_pc>=.
					replace xfcons`imod'_pc = max(y`imod'_pc,0) if xfcons`imod'_pc>=.
					...
					*non-food
    *				tobit xnfcons`imod'_pc xfcons0_pc xnfcons0_pc xdurables_pc `model' i.cluster [aweight=weight], ll(0) log // With cluster-level dummies 
					tobit xnfcons`imod'_pc xfcons0_pc xnfcons0_pc  `model2'          [aweight=weight], ll(0) log // No cluster-level dummies
					mata: cond(st_matrix("e(V)"))
					predict y`imod'_pc if xnfcons`imod'_pc>=.
	*				replace xnfcons`imod'_pc = exp(y`imod'_pc) if xnfcons`imod'_pc>=.
					replace xnfcons`imod'_pc = max(y`imod'_pc,0) if xnfcons`imod'_pc>=.
				...
				forvalues imod = 1/`M' {
					*food
	*				tobit xfcons`imod'_pc xfcons0_pc xnfcons0_pc xdurables_pc `model' i.cluster [aweight=weight], ll(0) log // With cluster-level dummies
					tobit xfcons`imod'_pc xfcons0_pc xnfcons0_pc  `model3'           [aweight=weight], ll(0) log // No cluster-level dummies
					mata: cond(st_matrix("e(V)"))
					predict y`imod'_pc if xfcons`imod'_pc>=.
	*				replace xfcons`imod'_pc = exp(y`imod'_pc) if xfcons`imod'_pc>=.
					replace xfcons`imod'_pc = max(y`imod'_pc,0) if xfcons`imod'_pc>=.
					...
					*non-food
    *				tobit xnfcons`imod'_pc xfcons0_pc xnfcons0_pc xdurables_pc `model' i.cluster [aweight=weight], ll(0) log // With cluster-level dummies 
					tobit xnfcons`imod'_pc xfcons0_pc xnfcons0_pc  `model3'          [aweight=weight], ll(0) log // No cluster-level dummies
					mata: cond(st_matrix("e(V)"))
					predict y`imod'_pc if xnfcons`imod'_pc>=.
	*				replace xnfcons`imod'_pc = exp(y`imod'_pc) if xnfcons`imod'_pc>=.
					replace xnfcons`imod'_pc = max(y`imod'_pc,0) if xnfcons`imod'_pc>=.
    ...
    *				reg xfcons`imod'_pc i.pxfcons0_pc i.pxnfcons0_pc i.pxdurables_pc `model' i.cluster [aweight=weight] // With cluster-level dummies
					reg xfcons`imod'_pc i.pxfcons0_pc i.pxnfcons0_pc `model'           [aweight=weight] // No cluster-level dummies
					predict y`imod'_pc if xfcons`imod'_pc>=.
	*				replace xfcons`imod'_pc = exp(y`imod'_pc) if xfcons`imod'_pc>=.
					replace xfcons`imod'_pc = max(y`imod'_pc,0) if xfcons`imod'_pc>=.
					drop y`imod'_pc
	...
	*				reg xnfcons`imod'_pc i.pxfcons0_pc i.pxnfcons0_pc i.pxdurables_pc `model' i.cluster [aweight=weight] // With cluster-level dummies
					reg xnfcons`imod'_pc i.pxfcons0_pc i.pxnfcons0_pc  `model'           [aweight=weight] // No cluster-level dummies
					predict y`imod'_pc if xnfcons`imod'_pc>=.
	*				replace xnfcons`imod'_pc = exp(y`imod'_pc) if xnfcons`imod'_pc>=.
					replace xnfcons`imod'_pc = max(y`imod'_pc,0) if xnfcons`imod'_pc>=.
					drop y`imod'_pc
				...
				forvalues imod = 1/`M' {
					*food
    *				reg xfcons`imod'_pc i.pxfcons0_pc i.pxnfcons0_pc i.pxdurables_pc `model' i.cluster [aweight=weight] // With cluster-level dummies
					reg xfcons`imod'_pc i.pxfcons0_pc i.pxnfcons0_pc  `model2'           [aweight=weight] // No cluster-level dummies
					predict y`imod'_pc if xfcons`imod'_pc>=.
	*				replace xfcons`imod'_pc = exp(y`imod'_pc) if xfcons`imod'_pc>=.
					replace xfcons`imod'_pc = max(y`imod'_pc,0) if xfcons`imod'_pc>=.
					drop y`imod'_pc
					*non-food
	*				reg xnfcons`imod'_pc i.pxfcons0_pc i.pxnfcons0_pc i.pxdurables_pc `model' i.cluster [aweight=weight] // With cluster-level dummies
					reg xnfcons`imod'_pc i.pxfcons0_pc i.pxnfcons0_pc `model2'           [aweight=weight] // No cluster-level dummies
					predict y`imod'_pc if xnfcons`imod'_pc>=.
	*				replace xnfcons`imod'_pc = exp(y`imod'_pc) if xnfcons`imod'_pc>=.
					replace xnfcons`imod'_pc = max(y`imod'_pc,0) if xnfcons`imod'_pc>=.
					drop y`imod'_pc
				...
				forvalues imod = 1/`M' {
					*food
    *				reg xfcons`imod'_pc i.pxfcons0_pc i.pxnfcons0_pc i.pxdurables_pc `model' i.cluster [aweight=weight] // With cluster-level dummies
					reg xfcons`imod'_pc i.pxfcons0_pc i.pxnfcons0_pc  `model3'           [aweight=weight] // No cluster-level dummies
					predict y`imod'_pc if xfcons`imod'_pc>=.
	*				replace xfcons`imod'_pc = exp(y`imod'_pc) if xfcons`imod'_pc>=.
					replace xfcons`imod'_pc = max(y`imod'_pc,0) if xfcons`imod'_pc>=.
					drop y`imod'_pc
					*non-food
	*				reg xnfcons`imod'_pc i.pxfcons0_pc i.pxnfcons0_pc i.pxdurables_pc `model' i.cluster [aweight=weight] // With cluster-level dummies
					reg xnfcons`imod'_pc i.pxfcons0_pc i.pxnfcons0_pc  `model3'           [aweight=weight] // No cluster-level dummies
					predict y`imod'_pc if xnfcons`imod'_pc>=.
	*				replace xnfcons`imod'_pc = exp(y`imod'_pc) if xnfcons`imod'_pc>=.
					replace xnfcons`imod'_pc = max(y`imod'_pc,0) if xnfcons`imod'_pc>=.
					drop y`imod'_pc
				...
				*run MI
				...
				mi register regular hh* cluster lnxfcons0_pc lnxnfcons0_pc pchild psenior cfcons_pc cnfcons_pc 
				mi register passive xcons_pc xfcons_pc
				*MI method selection
				if ("`smethod'"=="MImvn") {
					*Multi-variate normal imputation using MCMC
					mi impute mvn xfcons1_pc xfcons2_pc xfcons3_pc xfcons4_pc xnfcons1_pc xnfcons2_pc xnfcons3_pc xnfcons4_pc = i.pxfcons0_pc i.pxnfcons0_pc `model', add(`nI') burnin(1000)
				}
	...
	*				mi impute chained (regress) xfcons1_pc xfcons2_pc xfcons3_pc xfcons4_pc xnfcons1_pc xnfcons2_pc xnfcons3_pc xnfcons4_pc = xfcons0_pc xnfcons0_pc xdurables_pc hhsize pchild psenior i.hhsex i.hhwater hhcook_5 i.hhtoilet i.hhmaterial i.hhmode i.hhplot i.hhfood i.cluster i.hhmod, add(`nI') noimp report
					mi impute chained (regress) xfcons1_pc xfcons2_pc xfcons3_pc xfcons4_pc xnfcons1_pc xnfcons2_pc xnfcons3_pc xnfcons4_pc = i.pxfcons0_pc i.pxnfcons0_pc `model', add(`nI') report
				}
				else {
			...
			*build aggregates; but replace with originals if available
			...
			quiet: `mipre' replace xcons_pc = xfcons_pc 
	...
	*extract detailed results from estimation
	foreach smethod of local lmethod {
		...
		*extract consumption
		...
		file close fh
		*prepare stata file
		import delimited "`lc_sdTemp'/simd_`smethod'.txt", clear
		*summarize imputations for MI
		*quiet: reshape long hh, i(simulation imputation) j(hhid)
		preserve
		keep simulation imputation hh10001-hh6990010
		quiet:reshape long hh, i(simulation imputation) j(hhid)
		save part1, replace
		restore
		keep simulation imputation hh7000001- hh13390010
		quiet: reshape long hh, i(simulation imputation) j(hhid)
		append using part1
		erase part1.dta
		ren hh est
